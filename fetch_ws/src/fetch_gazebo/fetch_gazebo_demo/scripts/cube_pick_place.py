...
from moveit_msgs.msg import PlaceLocation, MoveItErrorCodes, Grasp, CollisionObject
...
#obj0_goal = (0, Pose(position=Point(x=1.114, y= -0.858, z= 0.65), orientation=Quaternion(x=0, y=0, z=0, w=1)))
    ...
    def getGraspableObject(self, goal_obj_x, goal_obj_y):
        ...
        for obj in self.objects:
            ...
            ## dimensions[0]: x coordinate of each imported object
            ...
            # get goal object
            if (( abs(obj.object.primitive_poses[0].position.x - goal_obj_x) > 0.07) and ( abs(obj.object.primitive_poses[0].position.y - goal_obj_y) > 0.07)):
...
if __name__ == "__main__":
    ...
    # set planning scene interface for collision objects
    scene = PlanningSceneInterface("base_link")

    # # set table as a collision object
    ...
    for i in range(len(init_pose)):
        sorted_init_pose.append(init_pose[i])
    ...
    for b in range(0, len(models)): #generate block objects
        ...
        object_pose = sorted_init_pose[b]
...
#         head_action.look_at(1.5, 0.0, 0.0, "map")
...
#             cube, grasps = grasping_client.getGraspableObject(sorted_init_pose[obj_i].position.x, sorted_init_pose[obj_i].position.y)
