...
class ImageListener:
    ...
    def __init__(self, network, dataset):
        ...
        self.prefix = prefix
        fusion_type = ''
        self.allowed_classes = ["003", "004", "005", "006", "009", "010", "011"]
        ...
        self.object_pub = rospy.Publisher('posecnn_objects', Class, queue_size=1)
        ...
        depth_sub = message_filters.Subscriber('/head_camera/depth_registered/image', Image, queue_size=10)
    ...
    def run_network(self):
        ...
        #publish objects' classes information
        class_msg = Class()
        ...
        for i in range(rois.shape[0]):
            ...
            cls = int(rois[i, 1])
            # if indexes[cls] not in self.allowed_classes:
            #     break
            if cls > 0 and rois[i, -1] > cfg.TEST.DET_THRESHOLD:
                ...
                name = self.dataset.classes[cls][1:]
                name = name + fusion_type
                indexes[cls] += 1
                name = name + '_%01d' % (indexes[cls])
                ...
                " ************** Start the custom code ************** " 
                # convey classes for collision objects
                pick_msg.name = name
                pick_msg.posestamped.header.frame_id = self.base_frame
                pick_msg.posestamped.header.stamp = rospy.Time.now()

                pick_msg.posestamped.pose.position.x = poses_refined[i, 4]
                pick_msg.posestamped.pose.position.y = poses_refined[i, 5]
                pick_msg.posestamped.pose.position.z = poses_refined[i, 6]
                pick_msg.posestamped.pose.orientation.x = poses_refined[i, 1]
                pick_msg.posestamped.pose.orientation.y = poses_refined[i, 2]
                pick_msg.posestamped.pose.orientation.z = poses_refined[i, 3]
                pick_msg.posestamped.pose.orientation.w = poses_refined[i, 0]


                self.tf_listener.waitForTransform(self.base_frame, "base_link", rospy.Time.now(), rospy.Duration(3))

                pick_msg.posestamped = self.tf_listener.transformPose("base_link", pick_msg.posestamped)
                class_msg.objects.append(pick_msg)

        print("===========================")
        print(len(class_msg.objects))
        for obj in class_msg.objects:
            print(obj.name)
        self.object_pub.publish(class_msg)
        print("===========================")
        " ************** Custom ************** " 
...
if __name__ == '__main__':
    ...
    #     print(i)
    #     print(dataset.model_texture_paths[i-1])
    # print("*****************************")
