...
if __name__ == "__main__":
    ...
    init_pose = [Pose(position=Point(x=0.93, y=-0.02, z=0.8)),
                 Pose(position=Point(x=0.88, y=0.2, z=0.8)),
                 Pose(position=Point(x=0.88, y=-0.2, z=0.8))] 
    
    obj_goal = [Pose(position=Point(x=0.88, y=0, z=1)),
                Pose(position=Point(x=0.88, y=0, z=1)),
                Pose(position=Point(x=0.88, y=0, z=1))]

    models = ['red_cube', 'blue_cube', 'green_cube']

    for b in range(0, len(models)): #spawn the objects
        object_path = os.path.join(pack_path, 'models/cubes', models[b] +'/model.sdf')
    ...
    while not rospy.is_shutdown():
        ...
        while not rospy.is_shutdown() and not cube_in_grapper:
            ...
            print("*******goal y index: ", obj_goal[0])
            print("*******goal object y: ", init_pose[0].position.y)

#            print(grasping_client.__dict__)
            cube, grasps = grasping_client.getGraspableObject(init_pose[0].position.x, init_pose[0].position.y)
...
#ssssssss head_camera_rgb_optical_frame
        ...
        while not rospy.is_shutdown() and cube_in_grapper:
            ...
            pose.pose.position = obj_goal[1].position
