...
class ImageListener:
    ...
    def run_network(self):
        ...
        objects_msg = Class()
        print(rois.shape[0], "checksljflksjdlf")
        ...
        for i in range(rois.shape[0]):
            ...
            if cls > 0 and rois[i, -1] > cfg.TEST.DET_THRESHOLD:
                ...
                print(i)
                pick_msg = EachObject()
                pick_msg.header.stamp = rospy.Time.now()
                pick_msg.header.frame_id = self.base_frame
                pick_msg.name = name
                pick_msg.pose.position.x = poses[i, 4]
                pick_msg.pose.position.y = poses[i, 5]
                pick_msg.pose.position.z = poses[i, 6]

                objects_msg.objects[i] = pick_msg

        print("===========================")
        print(len(object_msg.objects))
        print(object_msg.objects)
        print("===========================")
